...
export default function NetworkErrorElement({ message, reset }: MessageProp) {
        ...
        >
            <Box display={"flex"} gap={1} flexDirection={"column"}>
                <StyledTypography textAlign={"center"} variant={"h2"}>
                    {t("feedback.error.network.title")}
                </StyledTypography>
                <StyledTypography
                    textAlign={"center"}
                    maxWidth={700}
                    ...
                    variant={"h6"}
                >
                    {t("feedback.error.network.subtitle")}
                </StyledTypography>
                {message && (
                    <StyledTypography
                        overflow={"hidden"}
            ...
            {resetFn && (
                <StyledButton onClick={resetFn} variant="contained">
                    {t("feedback.error.network.reload")}
                </StyledButton>
            )}
        </Box>
    );
